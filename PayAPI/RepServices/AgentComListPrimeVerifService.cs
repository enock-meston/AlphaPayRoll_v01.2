...
{
    public interface AgentComListPrimeVerifService
    {

        public interface IAgentComListPrimeVerifService
        {
            Task<byte[]> GenerateListPrimeVerifAsync(string reportName, string reportType);


        }
        public class ClasListPrimeGenVerifService : IAgentComListPrimeVerifService
        {
            private readonly string connectionString;
            public ClasListPrimeGenVerifService(IConfiguration configuratrion)
            {
                connectionString = configuratrion.GetConnectionString("ApiConnection")!;
            }

            List<PrimeAgComVerif> itemList = new List<PrimeAgComVerif>();
            ...
            public async Task<byte[]> GenerateListPrimeVerifAsync(string reportName, string reportType)
            {
                ...
                using (IDbConnection oCon = new SqlConnection(connectionString))
                {
                    ...
                    var List = await oCon.QueryAsync<PrimeAgComVerif>("Ps_PrimeAgComVerif", commandType: CommandType.Text); ;
                ...
                rdlcReport.AddDataSource("dsPrimeAgComVerif", itemList);
