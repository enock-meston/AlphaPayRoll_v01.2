...
{
    public interface AgentComListPrimeService
    {

        public interface IAgentComListPrimeService
        {
            Task<byte[]> GenerateListPrimeAsync(string reportName, string reportType);


        }
        public class ClasListPrimeGenService : IAgentComListPrimeService
        {
            private readonly string connectionString;
            public ClasListPrimeGenService(IConfiguration configuratrion)
            {
                connectionString = configuratrion.GetConnectionString("ApiConnection")!;
            }

            List<AgentComPrime> itemList = new List<AgentComPrime>();
            ...
            public async Task<byte[]> GenerateListPrimeAsync(string reportName, string reportType)
            {
                ...
                using (IDbConnection oCon = new SqlConnection(connectionString))
                {
                    ...
                    var List = await oCon.QueryAsync<AgentComPrime>("Ps_ListePrimeAgentCom", commandType: CommandType.Text); ;
                ...
                rdlcReport.AddDataSource("AgentComPrime", itemList);
