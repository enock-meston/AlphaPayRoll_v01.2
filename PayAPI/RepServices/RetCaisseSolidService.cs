...
namespace PayAPI.RepServices
{
    ...
    public interface IRetCaisseSolidService
    {
        Task<byte[]> GenerateListAsync(string reportName, string reportType);


    }
    public class RetCaisseSolidService : IRetCaisseSolidService
    {
        private readonly string connectionString;
        public RetCaisseSolidService(IConfiguration configuratrion)
        {
        ...
        public async Task<byte[]> GenerateListAsync(string reportName, string reportType)
        {
            ...
            using (IDbConnection oCon = new SqlConnection(connectionString))
            {
                ...
                var List = await oCon.QueryAsync<RetenueModel>("Ps_ListRetCaisSolid", commandType: CommandType.StoredProcedure); ;
