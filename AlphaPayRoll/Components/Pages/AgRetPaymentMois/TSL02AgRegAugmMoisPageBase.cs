...
{
	public class TSL02AgRegAugmMoisPageBase : ComponentBase
	{
		...
		[Inject]
		protected ITSL02AgRegAugmMois oAgDonIntialMoisService { set; get; }
		...
		public List<TabPrmNivOne> oTSL550TpAugmRegulList { set; get; }
		...
		protected async Task SaveData(AgDonIntialMois item)
		{
			...
			else
			{
                ...
                {
                    await JSRuntime.InvokeVoidAsync("alert", "Entrer le montant  de la retenue SVP !");
                    return;
                }
			...
			try
			{
				...
				oResultat = await oAgDonIntialMoisService.GetUpdateRegAugmMoisResult(item);
				await JSRuntime.InvokeVoidAsync("alert", oResultat.Result);
				//oAgDonIntialMoisList = await oAgDonIntialMoisService.GetTSL02AgRegAugmMois();
		...
		protected override async Task OnInitializedAsync()
		{
				...
				try
				{
					...
					oTSL550TpAugmRegulList = (await oDonBaseService.GetDBListName("TSL550TpAugmRegul")).ToList();
					oAgDonIntialMoisList = await oAgDonIntialMoisService.GetTSL02AgRegAugmMoisByAgent(paramAgentId);
