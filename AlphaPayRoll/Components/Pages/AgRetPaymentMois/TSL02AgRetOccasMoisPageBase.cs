...
{
	public class TSL02AgRetOccasMoisPageBase : ComponentBase
	{
		...
		[Inject]
		protected ITSL02AgRetOccasMois oAgDonIntialMoisService { set; get; }
		...
		public List<TabPrmNivOne> oTSL550TpRetAutreList { set; get; }
		...
		protected async Task SaveData(AgDonIntialMois item)
		{
			...
			try
			{
				...
				oResultat = await oAgDonIntialMoisService.GetUpdateRetOccasMoisResult(item);
				await JSRuntime.InvokeVoidAsync("alert", oResultat.Result);
				oAgDonIntialMoisList = await oAgDonIntialMoisService.GetTSL02AgRetOccasMoisByAgent(paramAgentId);
		...
		protected override async Task OnInitializedAsync()
		{
			...
			try
			{
				...
				oTSL550TpRetAutreList = (await oDonBaseService.GetDBListName("TSL550TpRetAutre")).ToList();
				oAgDonIntialMoisList = await oAgDonIntialMoisService.GetTSL02AgRetOccasMoisByAgent(paramAgentId);
			}
			catch (Exception ex)
			{
