...
namespace AlphaPayRoll.Components.Pages.Promotion
{
    public class TRH05PromotionBase : ComponentBase
    {
        ...
        [Inject]
        protected ITRH05Promotion oTRH05PromotionService { get; set; }
        public List<TRH05Promotion> oTRH05PromotionList { set; get; }
        ...
        public TRH05Promotion oOneCotrat { set; get; }
        ...
        protected void ShowPopUp(int tPAction)
        {
            ...
            if (tPAction == 0)
            {
                modalTitle = " Promotion. Detail";
            }
            if (tPAction == 2)
            {
                modalTitle = "Ajouter Promotion. Detail";
                StyleButton = "btn btn-sm btn-primary ";
                ButtonCaption = "Sauvegarder";
            }
            else if (tPAction == 3)
            {
                modalTitle = "Supprimer Promotion. Detail";
        ...
        protected async Task ValidateData(int id)
        {
            try
            {
                var param = new ParamValidPromotion
                {
                    ID = id,
                ...
                var result = await oTRH05PromotionService.ValiderPromotion(param);
                await JSRuntime.InvokeVoidAsync("alert", "Message" + result.Result);
                oTRH05PromotionList = await oTRH05PromotionService.GetPromotionByMatricule(sMatricule);
        ...
        protected void EditData(TRH05Promotion item, int TpAction)
        {
        ...
        protected async Task SaveContrat(TRH05Promotion item)
        {
            ...
            try
            {
                ...
                oResultat = await oTRH05PromotionService.GetResutUpdate(item);
                await JSRuntime.InvokeVoidAsync("alert", oResultat.Result);
                oTRH05PromotionList = await oTRH05PromotionService.GetPromotionByMatricule(id);
        ...
        public void DonAgentSuite(TRH05Promotion pContrat)
        {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            try
            {
                ...
                oTRH05PromotionList = await oTRH05PromotionService.GetPromotionByMatricule(id);  // after search
