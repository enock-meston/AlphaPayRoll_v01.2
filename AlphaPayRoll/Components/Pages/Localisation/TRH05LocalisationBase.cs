...
namespace AlphaPayRoll.Components.Pages.Localisation
{
    public class TRH05LocalisationBase : ComponentBase
    {
        ...
        public TRH05Localisation oOneLocalisation { set; get; }

        public List<TRH05Localisation> oTRH05LocalisationList { set; get; }
        [Inject]
        protected ITRH05Localisation oTRH05LocalisationService { set; get; }
        ...
        protected void ShowPopUp(int tPAction)
        {
            if (tPAction == 0)
            {
                modalTitle = " Localisation Detail";
            }


            if (tPAction != 1)
            {
                pSubBranchID = oOneLocalisation.SBranchFromID;
                pBranchID = pSubBranchID.Substring(0, 2);

                pSubBranchIDTo = oOneLocalisation.SBranchToID;
            ...
            if (tPAction == 2)
            {
                modalTitle = "Modifier Localisation Detail";
                StyleButton = "btn btn-sm btn-primary ";
                ButtonCaption = "Sauvegarder";
            }
            else if (tPAction == 3)
            {
                modalTitle = "Supprimer Localisation Detail";
                StyleButton = "btn btn-sm btn-danger ";
                ButtonCaption = "Supprimer";
                oOneLocalisation.LModifBy = int.Parse(osessionService.UserId);
                oOneLocalisation.LModifOn = DateTime.Now;
                oOneLocalisation.Matricule = sMatricule;
            }

            if (tPAction == 1)
            {
                modalTitle = "Add Localisation";
                StyleButton = "btn btn-sm btn-primary";
                ButtonCaption = "Save";
                iTypeAction = tPAction;

                if (oOneLocalisation == null || string.IsNullOrEmpty(oOneLocalisation.Matricule))
                {
                    oOneLocalisation = new TRH05Localisation();
                    oOneLocalisation.ID = 0;
                    oOneLocalisation.Matricule = sMatricule;
                    oOneLocalisation.CreatBy = int.Parse(osessionService.UserId);
                    oOneLocalisation.CreatOn = DateTime.Now;
                    oOneLocalisation.FunctionID = 0;
                    oOneLocalisation.StatusID = 1;
                    oOneLocalisation.DateMut = DateTime.Today;
        ...
        protected async Task ValidateData(int id)
        {
            try
            {
                var param = new ParamValidLocalisation
                {
                    ID = id,
                ...
                var result = await oTRH05LocalisationService.ValiderLocalisation(param);
                ...
                if (oTRH02AgentList.Count > 0)
                {
                    ...
                    oTRH05LocalisationList = await oTRH05LocalisationService.GetList(sMatricule);
        ...
        protected void EditData(TRH05Localisation item, int TpAction)
        {

            iTypeAction = TpAction;
            oOneLocalisation = item;
        ...
        protected async Task SaveLocalisation(TRH05Localisation item)
        {
            ...
            try
            {
                ...
                oOneLocalisation.TpMaj = iTypeAction;
                //oOneLocalisation.UserId = osessionService.UserId;

                oResultat = new Resultat();

                oResultat = await oTRH05LocalisationService.GetUpdateResult(item);
                await JSRuntime.InvokeVoidAsync("alert", oResultat.Result);
                //oContratList = await oContratService.GetContractByMatricule(id);
                oTRH05LocalisationList = await oTRH05LocalisationService.GetListAll();
        ...
        public void BranchChanged(string Value)
        {
            ...
            oOneLocalisation.SBranchFromID = "0";
        ...
        {
            pSubBranchID = Value;
            oOneLocalisation.SBranchFromID = pSubBranchID;
        }
        ...
        public void BranchChangedTo(string Value)
        {
            ...
            oOneLocalisation.SBranchToID = "0";
        ...
        public void SubBranchChangedTo(string Value)
        {
            ...
            oOneLocalisation.SBranchToID = pSubBranchIDTo;
        }
