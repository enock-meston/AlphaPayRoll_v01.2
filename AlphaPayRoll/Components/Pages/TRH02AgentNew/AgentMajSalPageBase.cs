...
namespace AlphaPayRoll.Components.Pages.TRH02AgentNew
{
    public class AgentMajSalPageBase : ComponentBase
    {
        ...
        public void ShowPopUp(int tPAction)
        {
            ...
            {
                modalTitle = "Delete Agent Details";
        ...
        public async Task SaveTRH02Agent(ClassTRH02Agent item)
        {
            ...
            if (iTypeAction == 3)
            {
                if (!await JSRuntime.InvokeAsync<bool>("confirm", $"Do you really want to Delete this Agent ?"))
                    return;
            }
            else

            {
                ...
                if (string.IsNullOrWhiteSpace(pBranchLocID) || pBranchLocID == "0")
                {
            ...
            try
            {
                ...
                // Modal uses pBranch* for InputSelect; keep agent DTO in sync before POST (API / SP read BranchLocID, etc.)
                oOneTRH02Agent.BranchLocID = pBranchLocID ?? string.Empty;
                oOneTRH02Agent.BranchCpteID = pBranchCpteID ?? string.Empty;
                oOneTRH02Agent.SBranchLocID = pSubBranchLocID ?? string.Empty;
                oOneTRH02Agent.SBranchCpteID = pSubBranchCpteID ?? string.Empty;

                oResultat = new Resultat();

                oResultat = await oTRH02AgentService.GetResutMajSalaire(oOneTRH02Agent);
