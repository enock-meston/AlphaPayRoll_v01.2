...
{
    public class ReportTestBase : ComponentBase
    {
        [Inject]
        ...
        [Inject]
        protected IListPayByBranch oListPayByBranchService { set; get; }
        ...
        public List<ListPayByBranch> SalaireList { get; set; } = new();

        /// <summary>Whole amounts with comma thousands (e.g. 1,094,040,000), no decimal places.</summary>
        protected static string FmtAmount0(decimal value) =>
            value.ToString("#,##0", CultureInfo.InvariantCulture);

        // `Branch` comes from the SP as: "<branch name>  Mois : <MM/YYYY>"
        ...
        public async Task searchByBranch(string? branchId)
        {
            var effectiveBranchId = string.IsNullOrWhiteSpace(branchId) ? pBranchID : branchId;

            // Keep component state in sync with the UI selection.
            pBranchID = effectiveBranchId;
            SalaireList = await oListPayByBranchService.GetListPayByBranch(effectiveBranchId);

            // Ensures UI updates immediately when triggered from InputSelect change.
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            try
            {
                ...
                SalaireList = await oListPayByBranchService.GetListPayByBranch(pBranchID);
