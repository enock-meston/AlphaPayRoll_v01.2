...
{
    public class ContratPageBase : ComponentBase
    {
        [Inject]
        ...
        public ClasContrat oOneCotrat { set; get; }
        ...
        protected async Task ShowPopUp(int tPAction)
        {
            ...
            if (tPAction == 2)
            {
                modalTitle = "Modification Contrat Detail";
        ...
        protected async Task ValidateData(int id)
        {
            try
            {
                ...
                var result = await oContratService.ValiderCotrat(param);
        ...
        //protected async Task SuspendContract(int id,string NumContratF)
        //{
        //    try
        //    {
        ...
        //        var param = new ParamSuspendContract
        //            {
        //                ID = id,
        //                UserID = int.Parse(osessionService.UserId),
        //                Matricule = sMatricule,
        //                NumContrat = NumContratF,
        //                TpMaj = 1
        //            };

        //            var result = await oContratService.ContractSuspesion(param);
        //            await JSRuntime.InvokeVoidAsync("alert", "Message" + result.Result);
        //            oContratList = await oContratService.GetContractByMatricule(sMatricule);
        ...
        //    {
        //        await JSRuntime.InvokeVoidAsync("alert", $"An error occurred: {ex.Message}");
        //        Console.WriteLine($"Error in Suspession of contract: {ex}");
        //    }
        //}



        protected void EditData(ClasContrat item, int TpAction)
        {
        ...
        protected async Task SaveContrat(ClasContrat item)
        {
            ...
            else if (oOneCotrat.Notes == null || oOneCotrat.Notes == "")
            {
                await JSRuntime.InvokeVoidAsync("alert", "Le Observations est obligatoire");
                return;
            }
            else if (oOneCotrat.StatusID == 0 || oOneCotrat.StatusID == null)
            {
                await JSRuntime.InvokeVoidAsync("alert", "Le Statut contrat est obligatoire");
                return;
            }
            else if (oOneCotrat.Categorie == 0 || oOneCotrat.Categorie == null)
            {
                await JSRuntime.InvokeVoidAsync("alert", "Le Statut contrat est obligatoire");
                return;
            }
            else if (oOneCotrat.ContraPeriod == "" || oOneCotrat.ContraPeriod == null)
            {
                await JSRuntime.InvokeVoidAsync("alert", "Le Statut contrat est obligatoire");
                return;
            }
        ...
        public void OnContractTypeChanged(int value)
        {
            oOneCotrat.Categorie = value;

            if (value == 1 || value == 2)
            {
                oOneCotrat.ContraPeriod = "3 Mois";
            }
            else
            {
                oOneCotrat.ContraPeriod = ""; 
            }
            StateHasChanged();
        }
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            try
            {
                ...
                oContratList = await oContratService.GetContractByMatricule(id);  // after search
